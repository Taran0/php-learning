...
$sql = "DELETE FROM projects WHERE id='{$actualID}'";

$execute = $connectDB->query($sql);

if ($execute) {
    //self - nem nyit új tabot
    echo "<script>window.open('start.php?status=del', '_self')</script>";
}
}
?>
...
?>
    <form action="delete.php?id=<?php echo $id; ?>" method="post" class="window">
